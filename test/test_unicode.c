...
SUIT(unicode, test_finalize, NULL);

#define TEST_UNICODE(name, result, encoded)                               \
    TEST(unicode, name) {                                                 \
        const char *text = "[\"" encoded "\"]";                           \
        plain_json_ErrorType status =                                     \
            plain_json_parse(&context, alloc_config, (u8*)text, strlen(text)); \
...
#define TEST_UNICODE_VALUE(name, raw, expected) \
    TEST(unicode, name) { \
        const char *text = "[\"" raw "\"]"; \
        plain_json_ErrorType status = plain_json_parse(&context, alloc_config, (u8*)text, strlen(text)); \
        ...
        test_assert_string_eq((char*)plain_json_get_string(&context, token->value.string_index), expected); \
}

TEST_UNICODE_VALUE(parse_utf16_surrogate, "\\uD83D\\uDE11", "\xF0\x9F\x98\x91") /* 😑 */
TEST_UNICODE_VALUE(parse_utf16_misc, "Bear emoji: \\u0295\\u00b7\\u0361\\u1d25\\u00b7\\u0294", "Bear emoji: \xca\x95\xc2\xb7\xcd\xa1\xe1\xb4\xa5\xc2\xb7\xca\x94") /* ʕ·͡ᴥ·ʔ */
